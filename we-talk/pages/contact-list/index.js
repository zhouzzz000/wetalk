...
var app = new Vue({
	...
	methods: {
		...
		tap2chat(chatUserId,index){
			...
			// });
			var friend = null;
			var friendInfoLlist = this.friendInfoList;
			for(var i = 0; i< friendInfoLlist.length; i++)
			{
				if(friendInfoLlist[i].id == chatUserId)
				{
					friend = friendInfoLlist[i];
					break;
				}
			}
			mui.openWindow(
